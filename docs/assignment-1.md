...
Arguments: `int socket(int domain, int type, int protocol);`
...
Arguments: `int bind(int sockfd, const struct sockaddr *addr, socklen_t addrlen);`
...
Arguments: `int listen(int sockfd, int backlog);`
...
Arguments: `int accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen);`

#### Direct Output with dup2()
Arguments: `int dup2(int oldfd, int newfd);`

#### Execute Program with execve()
Arguments: `int execve(const char *pathname, char *const argv[], char *const envp[]);`
